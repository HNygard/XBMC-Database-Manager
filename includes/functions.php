<?php
	...
	function PrintContent($id)
	{
		...
		switch ($_GET["view"])
		{
			case "movies":
				...
				$col2[6] = $col2[6] . " minutes";					#Add "minutes" after runtime
	...
	function PrintOptions($id)
	{
		...
		$page = $_GET['page'];
		$id = $_GET['id'];
		$q = "SELECT c00,playCount,strPath,strFileName FROM movieview WHERE idMovie = " . $id;
		$temp = dbquery($q);
		while ($row = $temp->fetch (PDO::FETCH_NUM))
			$col2 = $row;	
		/*foreach (dbquery($q) as $temp)
		...
		<table border="1">
			<?php
				...
				foreach ($col1 as $temp)
				{
					echo "<tr><th>$temp</th><td>$col2[$i]</td><td><button class=\"editbutton\" id=\"$temp\" type=\"button\" value=\"$col2[$i]\">Edit</button></td></tr>";
					$i++;
				}
			?>
		</table>
